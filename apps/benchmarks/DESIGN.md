...
3. [ベンチマーク計測設計](#ベンチマーク計測設計準備フェーズと計測フェーズの分離)
4. [ベンチ姿勢](#ベンチ姿勢-これを満たすと商用レベル)
5. [ターゲット比較マトリクス](#ターゲット比較マトリクス例キーワードのみ)
6. [代表ベンチ案](#代表ベンチ案52本から厳選)
7. [ハーネス設計](#ハーネスcli仕様と-json-出力)
8. [ベンチランナ雛形](#ベンチランナ雛形harnessbench_runnernyash)
9. [WASM/PHI/Effects/Contracts](#wasmphieffectscontracts-の特別考慮)
10. [CI統合](#ci-で市販アプリっぽくするポイント)
...
## ベンチマーク計測設計（準備フェーズと計測フェーズの分離）

### 🎯 計測の正確性を保つための設計原則

**重要**: ベンチマークは**準備が終わってから**計測する！

現状の問題：
```bash
# ❌ 間違った計測（すべて含まれる）
time ./target/release/hako program.nyash
# ↑ パース + MIR生成 + コンパイル + 実行 全部含まれてる！
```

### 📋 二段階計測モデル

#### 1️⃣ **準備フェーズ**（計測しない・1回のみ）

| バックエンド | 準備内容 | 時間 |
|------------|---------|------|
| **VM** | なし（準備不要） | 0ms |
| **LLVM** | MIR → LLVM IR → .oファイル → 実行ファイル生成 | ~700ms |
| **WASM** | MIR → WASM → .wasmファイル生成 | 未計測 |
...
#### 2️⃣ **計測フェーズ**（ここだけ計測・N回実行）

| バックエンド | 実行内容 | 時間（例） |
|------------|---------|----------|
| **VM** | VM実行 | 3-4ms |
| **LLVM** | 生成済み実行ファイルを実行 | 2ms |
| **WASM** | wasmtime実行 | 未計測 |

**実測データ**（01_counter.nyash）:
```bash
# LLVM（コンパイル込み・間違った計測）
real: 705ms

# LLVM（実行ファイル直接実行・正しい計測）
real: 2ms  ← VM（3-4ms）より速い！
...
- ✅ **終了時間確定**（指定秒数で必ず終わる）
- ✅ **統計的に安定**（十分なサンプル数）
- ✅ **warmup自然**（最初の数回は遅くても平均化される）
- ✅ **スループット明確**（ops/sec）

**CLI設計**:
```bash
# 高速テスト（1秒間）
./tools/bench_unified.sh --duration 1

# 標準ベンチ（10秒間・デフォルト）
./tools/bench_unified.sh --duration 10

# 本格ベンチ（60秒間）
./tools/bench_unified.sh --duration 60
```
...
```bash
# 1000回実行
./tools/bench_unified.sh --iterations 1000
```
...
```json
{
  "benchmark": "01_counter.nyash",
  "backend": "llvm",
  "preparation": {
    "compile_time_ms": 705,
    "executable_path": "tmp/bench_counter_llvm"
  },
  "measurement": {
    "duration_sec": 10,
    "iterations": 5000,
...
1. **準備フェーズ**: 1回のみ実行、時間を記録（参考情報）
2. **計測フェーズ**: 指定時間内で繰り返し実行、統計計算
3. **結果の分離**: `preparation` と `measurement` を分けて記録
4. **キャッシュ**: 同じソースなら準備フェーズをスキップ可能（ハッシュチェック）
...
| 2025-10-02 | ベンチマーク計測設計追加（準備/計測フェーズ分離、固定時間方式・秒数指定） |
