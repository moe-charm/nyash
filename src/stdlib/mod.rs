...
impl BuiltinStdlib {
    ...
    fn register_nyashstd(&mut self) {
        ...
        nyashstd
            .static_boxes
            .insert("string".to_string(), Self::create_string_box());

        // integer static box
        nyashstd
            .static_boxes
            .insert("integer".to_string(), Self::create_integer_box());

        // bool static box
        nyashstd
            .static_boxes
            .insert("bool".to_string(), Self::create_bool_box());

        // array static box
        nyashstd
            .static_boxes
            .insert("array".to_string(), Self::create_array_box());

        // console static box
        nyashstd
            .static_boxes
            .insert("console".to_string(), Self::create_console_box());
    ...
    fn create_string_box() -> BuiltinStaticBox {
        ...
        string_box.methods.insert("create".to_string(), |args| {
            ...
            if let Some(string_arg) = args[0].as_any().downcast_ref::<StringBox>() {
                ...
                Err(RuntimeError::TypeError {
                    message: format!(
                        "string.create() expects string argument, got {:?}",
                        args[0].type_name()
                    ),
                })
            }
        ...
        string_box.methods.insert("upper".to_string(), |args| {
            ...
            if let Some(string_arg) = args[0].as_any().downcast_ref::<StringBox>() {
                ...
                Err(RuntimeError::TypeError {
                    message: format!(
                        "string.upper() expects string argument, got {:?}",
                        args[0].type_name()
                    ),
                })
            }
    ...
    fn create_integer_box() -> BuiltinStaticBox {
        ...
        integer_box.methods.insert("create".to_string(), |args| {
            ...
            if let Some(int_arg) = args[0].as_any().downcast_ref::<IntegerBox>() {
                ...
                Err(RuntimeError::TypeError {
                    message: format!(
                        "integer.create() expects integer argument, got {:?}",
                        args[0].type_name()
                    ),
                })
            }
    ...
    fn create_bool_box() -> BuiltinStaticBox {
        ...
        bool_box.methods.insert("create".to_string(), |args| {
            ...
            if let Some(bool_arg) = args[0].as_any().downcast_ref::<BoolBox>() {
                ...
                Err(RuntimeError::TypeError {
                    message: format!(
                        "bool.create() expects bool argument, got {:?}",
                        args[0].type_name()
                    ),
                })
            }
