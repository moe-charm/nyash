use super::types::{
    as_float, as_int, classify_tag, cmp_eq_ne_any, i64_to_ptr, map_mirtype_to_basic, map_type,
    to_bool, to_i64_any,
};
...
use inkwell::values::{
    BasicValueEnum, FloatValue, FunctionValue, IntValue, PhiValue, PointerValue,
};
use inkwell::AddressSpace;
use std::collections::HashMap;

pub(crate) fn lower_instruction<'ctx>(
    inst: &MirInstruction,
    codegen: &CodegenContext<'ctx>,
    func: &MirFunction,
    bid: BasicBlockId,
    vmap: &mut HashMap<ValueId, BasicValueEnum<'ctx>>,
    allocas: &mut HashMap<ValueId, PointerValue<'ctx>>,
    entry_builder: &Builder<'ctx>,
    alloca_elem_types: &mut HashMap<ValueId, BasicTypeEnum<'ctx>>,
    ...
    match inst {
        MirInstruction::NewBox {
            ...
            match (box_type.as_str(), args.len()) {
                ...
                (_, n) if n == 1 || n == 2 => {
                    ...
                    let fnty =
                        i64t.fn_type(&[i64t.into(), i64t.into(), i64t.into(), i64t.into()], false);
                    let callee = codegen
                        .module
                ...
                _ => {
                    ...
                    if !args.is_empty() {
                        return Err(
                            "NewBox with >2 args not yet supported in LLVM lowering".to_string()
                        );
                    }
                    let type_id = *box_type_ids.get(box_type).unwrap_or(&0);
                    let i64t = codegen.context.i64_type();
        ...
        MirInstruction::BoxCall {
                    ...
                    match method.as_str() {
                        "get" => {
                            ...
                            let callee = codegen
                                ...
                                .unwrap_or_else(|| {
                                    codegen.module.add_function("nyash_array_get_h", fnty, None)
                                });
                            let call = codegen
                                .builder
                        ...
                        "set" => {
                            ...
                            let callee = codegen
                                ...
                                .unwrap_or_else(|| {
                                    codegen.module.add_function("nyash_array_set_h", fnty, None)
                                });
                            let _ = codegen
                                .builder
                                .build_call(
                        ...
                        "push" => {
                            ...
                            let val_i = match val_v {
                                ...
                                _ => {
                                    return Err(
                                        "array.push value must be int or handle ptr".to_string()
                                    )
                                }
                            };
                            let fnty = i64t.fn_type(&[i64t.into(), i64t.into()], false);
            ...
            if let Some(mid) = method_id {
                ...
                if use_f64_ret {
                    ...
                    let callee = codegen
                        ...
                        .unwrap_or_else(|| {
                            codegen
                                .module
                                .add_function("nyash_plugin_invoke3_f64", fnty, None)
                        });
                    let tid = i64t.const_int(type_id as u64, true);
                ...
                let classify =
                    |vid: ValueId| -> Option<i64> { vmap.get(&vid).map(|v| classify_tag(*v)) };
                if args.len() >= 1 {
                        ...
                        if let Some(mt) = func.metadata.value_types.get(d) {
                            match mt {
                                ...
                                | crate::mir::MirType::Unknown => {
                                    ...
                                    let pty =
                                        codegen.context.i8_type().ptr_type(AddressSpace::from(0));
                                    let ptr = codegen
                                        .builder
        ...
        MirInstruction::ExternCall {
                ...
                || (iface_name == "env.debug" && method_name == "trace")
            {
                ...
                if args.len() < 2 {
                    return Err(
                        "env.future.spawn_instance expects at least (recv, method_name)"
                            .to_string(),
                    );
                }
                let i64t = codegen.context.i64_type();
                ...
                let out_ptr: PointerValue = if args.len() == 1 {
                    ...
                    let callee = codegen
                        ...
                        .unwrap_or_else(|| {
                            codegen
                                .module
                                .add_function("nyash.env.box.new_i64x", fnty, None)
                        });
                    ...
                    if args.len() >= 2 {
                        ...
                        a1 = match bv {
                            ...
                            BasicValueEnum::FloatValue(fv) => {
                                let fnty =
                                    i64t.fn_type(&[codegen.context.f64_type().into()], false);
                                let callee = codegen
                                    .module
                            ...
                            _ => {
                                return Err("unsupported arg value for env.box.new".to_string());
                            }
                        };
                    }
                    let mut a2 = i64t.const_zero();
        ...
        MirInstruction::Compare { dst, op, lhs, rhs } => {
            ...
            let out = if let (Some(li), Some(ri)) = (as_int(lv), as_int(rv)) {
                ...
                codegen
                    ...
                    .map_err(|e| e.to_string())?
                    .into()
            } else if let (BasicValueEnum::PointerValue(lp), BasicValueEnum::PointerValue(rp)) =
                (lv, rv)
            {
            ...
            } else if let (BasicValueEnum::PointerValue(lp), BasicValueEnum::IntValue(ri)) =
                (lv, rv)
            {
                use crate::mir::CompareOp as C;
                match op {
            ...
            } else if let (BasicValueEnum::IntValue(li), BasicValueEnum::PointerValue(rp)) =
                (lv, rv)
            {
                use crate::mir::CompareOp as C;
                match op {
...
pub(crate) fn lower_terminator<'ctx>(
    ...
    match term {
        MirInstruction::Return { value } => match (&func.signature.return_type, value) {
                ...
                for (_dst, phi, inputs) in list {
                    if let Some((_, in_vid)) = inputs.iter().find(|(pred, _)| pred == &bid) {
                        ...
                        match val {
                            ...
                            BasicValueEnum::PointerValue(pv) => phi.add_incoming(&[(&pv, pred_bb)]),
                            _ => {
                                return Err("unsupported phi incoming value (then)".to_string());
                            }
                        }
                    }
                }
                ...
                for (_dst, phi, inputs) in list {
                    if let Some((_, in_vid)) = inputs.iter().find(|(pred, _)| pred == &bid) {
                        ...
                        match val {
                            ...
                            BasicValueEnum::PointerValue(pv) => phi.add_incoming(&[(&pv, pred_bb)]),
                            _ => {
                                return Err("unsupported phi incoming value (else)".to_string());
                            }
                        }
                    }
                }
