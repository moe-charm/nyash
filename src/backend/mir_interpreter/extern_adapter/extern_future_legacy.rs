// extern_future_legacy.rs — env.future.* handlers (unified for legacy + plugin only builds)
...
pub fn register(map: &mut HashMap<(String, String), super::HandlerFn>) {
    fn value_as_future(value: &VMValue) -> Option<FutureBox> {
        match value {
            VMValue::Future(fut) => Some(fut.clone()),
            VMValue::BoxRef(b) => b.as_any().downcast_ref::<FutureBox>().cloned(),
            _ => None,
        }
    }
    ...
    map.insert(("env.future".into(), "set".into()), |args: &[VMValue]| {
        if args.len() < 2 {
            return Err(VMError::InvalidInstruction("env.future.set requires 2 args".into()));
        }
    ...
    map.insert(("env.future".into(), "await".into()), |args: &[VMValue]| {
        let first = args
            .get(0)
            .ok_or_else(|| VMError::InvalidInstruction("env.future.await requires 1 arg".into()))?;
    ...
    map.insert(("env.future".into(), "spawn_instance".into()), |args: &[VMValue]| {
        ...
        let _scheduled = crate::runtime::global_hooks::spawn_task(&name, Box::new(move || {
            ...
            let out = match cb {
                VMValue::BoxRef(bx) => crate::runtime::method_router_box::route(
                    &mut vm,
                    &VMValue::BoxRef(bx),
                    "call",
                    &match argv {
