                ...
                if let Some(inst) = bref.as_any().downcast_ref::<crate::instance_v2::InstanceBox>() {
                    ...
                    if inst.class_name == "JsonParser" && fname == "length" {
                        if let Some(tokens_shared) = inst.get_field("tokens") {
                            let _tokens_box: Box<dyn crate::box_trait::NyashBox> = tokens_shared.share_box();
                            #[cfg(feature = "legacy-boxes")]
                            if let Some(arr) = _tokens_box.as_any().downcast_ref::<crate::boxes::array::ArrayBox>() {
                    ...
                    if let Some(nv) = inst.get_field_ng(&fname) {
                        if super::VmConfig::global().call_arg_trace {
                            eprintln!("[vm-getfield] class={} name={} nv={:?}", inst.class_name, fname, nv);
                        }
                        if super::VmConfig::global().general_trace && inst.class_name == "JsonToken" {
