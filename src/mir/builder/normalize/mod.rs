...
pub fn normalize_length_call(
    ...
    if let Callee::Method { box_name: ref callee_box, method, receiver: Some(r), .. } = callee.clone() {
        if callee_box == box_name
            && (method.as_str() == "size" || method.as_str() == "len" || method.as_str() == "length")
            && args.is_empty()
        {
            let recv_local = r;
...
/// - MapBox.(size|len|length)    → Extern("nyrt.map.size")(recv)
...
pub fn apply_all(builder: &mut MirBuilder, callee: &mut Callee, args: &mut Vec<ValueId>) {
    ...
    let _ = crate::mir::builder::normalize::map_length::normalize_map_length_call(builder, callee, args);
