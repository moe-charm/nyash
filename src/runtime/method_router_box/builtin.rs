...
pub fn try_route_builtin_box(
                ...
                "open" => {
                    if args.len() != 2 {
                        return Err(VMError::InvalidInstruction(
                            crate::common::diagnostics::msg::no_method_arity(
                                "FileBox",
                                "open",
                                args.len(),
                                &[2],
            ...
            if let Some(slot) = crate::runtime::type_registry::resolve_slot_by_name(
                ...
                let res = match slot as u32 {
                    ...
                    501 => {
                        ...
                        if let Some(recv) = &cb.receiver {
                            ...
                            crate::runtime::method_router_box::route(
                                _interp, &recv_vm, &cb.method, &argv,
                            )
                        } else {
                            Err(VMError::InvalidInstruction(
                                "CallableBox without receiver is not supported yet".into(),
                            ))
                        }
                    }
                    502 => {
                            ...
                            if let Some(recv) = &cb.receiver {
                                if let Some(p) = recv
                                    ...
                                    for v in &flat_vm {
                                        match v {
                                            VMValue::BoxRef(bx) => {
                                                if crate::runtime::type_registry::is_core_box(
                                                    bx.type_name(),
                                                ) {
                                                    let h =
                                                        crate::runtime::host_handles::to_handle_arc(
                                                            bx.clone(),
                                                        );
                                    ...
                                    let name =
                                        format!("callable.callAsync({}.{}#{})", typ, method, inst);
                                    let _scheduled = crate::runtime::global_hooks::spawn_task(
                                        &name,
                                        Box::new(move || {
                                    ...
                                    let fut_clone = fut.clone();
                                    let name =
                                        format!("callable.callAsync(builtin.{})", method_sched);
                                    let _scheduled = crate::runtime::global_hooks::spawn_task(
                                        &name,
                                        Box::new(move || {
                                            let mut vm = crate::backend::mir_interpreter::MirInterpreter::new();
                                            match crate::runtime::method_router_box::route(
                                                &mut vm,
                                                &recv_vm,
                                                &method_sched,
                                                &argv_vm,
                                                ...
                                                Err(_) => fut_clone.set_result(Box::new(
                                                    crate::box_trait::StringBox::new(
                                                        "invoke_failed",
                                                    ),
                                                )),
                                            }
                    ...
                    _ => Err(VMError::InvalidInstruction(
                        crate::common::diagnostics::msg::unknown_slot("CallableBox", method, slot),
                    )),
                }?;
                return Ok(Some(res));
            }
            return Err(crate::vm_ops::boxcall::unknown_method_err(
                "CallableBox",
                method,
                args.len(),
            ));
    ...
    if bx.type_name() == "ArrayBox" {
        ...
        if let Some(arr) = bx.as_any().downcast_ref::<crate::boxes::array::ArrayBox>() {
            ...
            let res = if let Some(slot) =
                crate::runtime::type_registry::resolve_slot_by_name("ArrayBox", method, args.len())
            {
                match slot as u32 {
                    100 => {
                        // get(index)
                        ...
                        Ok(if idx < items.len() {
                            VMValue::from_nyash_box(items[idx].clone_box())
                        } else {
                            VMValue::from_nyash_box(
                    ...
                    103 => {
                        ...
                        let end = args[1]
                            .as_integer()
                            .unwrap_or(arr.items.read().unwrap().len() as i64);
                    ...
                    _ => Err(VMError::InvalidInstruction(
                        crate::common::diagnostics::msg::unknown_slot("ArrayBox", method, slot),
                    )),
                }
            } else {
                Err(crate::vm_ops::boxcall::unknown_method_err(
                    "ArrayBox",
                    method,
                    args.len(),
                ))
    ...
    if bx.type_name() == "MapBox" {
        ...
        if let Some(mp) = bx.as_any().downcast_ref::<crate::boxes::map_box::MapBox>() {
            ...
            let res = if let Some(slot) =
                crate::runtime::type_registry::resolve_slot_by_name("MapBox", method, args.len())
            {
                match slot as u32 {
                    ...
                    211 => {
                            ...
                            .downcast_ref::<crate::boxes::callable::CallableBox>()
                        {
                            ...
                            let args_vm =
                                VMValue::BoxRef(std::sync::Arc::from(args[1].to_nyash_box()));
                            crate::runtime::method_router_box::route(
                                _interp,
                                &cb_vm,
                                "callAsync",
                            ...
                            Err(VMError::InvalidInstruction(
                                "Map.callAsync: value is not CallableBox".into(),
                            ))
                        }
                    }
                    _ => Err(VMError::InvalidInstruction(
                        crate::common::diagnostics::msg::unknown_slot("MapBox", method, slot),
                    )),
                }
            } else {
                Err(crate::vm_ops::boxcall::unknown_method_err(
...
pub fn try_route_string_primitive(
    ...
    if let VMValue::String(s) = receiver {
        ...
        if let Some(slot) =
            crate::runtime::type_registry::resolve_slot_by_name("StringBox", method, args.len())
        {
            let res = match slot as u32 {
                300 => Ok(Some(VMValue::Integer(hako_core_string::length_bytes(s)))),
                301 => {
                    let start = args
                        .get(0)
                    ...
                    let end = args
                        .get(1)
                        .map(|v| v.as_integer().unwrap_or(hako_core_string::length_bytes(s)))
                        .unwrap_or(hako_core_string::length_bytes(s));
                    Ok(Some(VMValue::String(hako_core_string::substring_bytes(
                        s, start, end,
                    ))))
                }
                303 => {
                    ...
                    let from = args
                        ...
                        .map(|v| v.as_integer().unwrap_or(0))
                        .unwrap_or(0);
                    Ok(Some(VMValue::Integer(hako_core_string::index_of(
                        s, &needle, from,
                    ))))
                }
                313 => {
                    ...
                    let from = args
                        ...
                        .map(|v| v.as_integer().unwrap_or(hako_core_string::length_bytes(s)))
                        .unwrap_or(hako_core_string::length_bytes(s));
                    Ok(Some(VMValue::Integer(hako_core_string::last_index_of(
                        s, &needle, from,
                    ))))
                }
                314 => {
                    let idx = args
                        ...
                        .map(|v| v.as_integer().unwrap_or(0))
                        .unwrap_or(0);
                    Ok(Some(VMValue::String(hako_core_string::char_at_byte(
                        s, idx,
                    ))))
                }
                302 => {
                ...
                304 => {
                    ...
                    Ok(Some(VMValue::String(hako_core_string::replace_all(
                        s, &from, &to,
                    ))))
                }
                ...
                310 => {
                    let needle = args.get(0).map(|v| v.to_string()).unwrap_or_default();
                    Ok(Some(VMValue::Bool(s.starts_with(&needle))))
                }
                311 => {
                    let needle = args.get(0).map(|v| v.to_string()).unwrap_or_default();
                    Ok(Some(VMValue::Bool(s.ends_with(&needle))))
                }
                312 => Ok(Some(VMValue::Bool(hako_core_string::is_empty(s)))),
                _ => Err(VMError::InvalidInstruction(
                    crate::common::diagnostics::msg::unknown_slot("StringBox", method, slot),
                )),
            }?;
            return Ok(res);
        } else {
