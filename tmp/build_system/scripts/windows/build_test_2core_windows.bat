@echo off
echo === NyaMesh Editor 2Core Integration Test Windows Build ===
echo EditorCore + SettingsCore Intent-driven Test
...
if not exist build-test-2core-windows mkdir build-test-2core-windows
cd build-test-2core-windows

echo Starting CMake configuration...

:: Copy and use custom CMakeLists file
copy /Y "..\CMakeLists_Test_2Core.txt" "..\CMakeLists.txt"
...
:: Build execution
cmake --build . --config Release --target Test_2Core_Editor_Settings
...
copy /Y "Test_2Core_Editor_Settings.exe" "..\windows_exe\"
...
echo.
echo === BUILD SUCCESS ===
echo Output file: build-test-2core-windows\Test_2Core_Editor_Settings.exe
echo Final location: windows_exe\Test_2Core_Editor_Settings.exe
echo.
echo How to run:
echo cd windows_exe
echo Test_2Core_Editor_Settings.exe
echo.
goto end
