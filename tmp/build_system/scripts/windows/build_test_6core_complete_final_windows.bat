@echo off
echo === NyaMesh Editor 6Core Complete Final Integration Test Windows Build ===
echo EditorCore + SettingsCore + FileSystemCore + UICore + LocalizationCore + QuickAccessCore (6 cores complete, Header Compliant)
...
if not exist build-test-6core-complete-final-windows mkdir build-test-6core-complete-final-windows
cd build-test-6core-complete-final-windows

echo Starting CMake configuration (SAFE MODE - No root overwrite)...

:: SAFE: Copy SAFE CMakeLists to BUILD directory (NOT root!)
copy /Y "..\CMakeLists_Test_6Core_Complete_Final.txt" ".\CMakeLists.txt"
if %errorlevel% neq 0 (
...
:: Build execution
cmake --build . --config Release --target Test_6Core_Complete_Final
...
copy /Y "Test_6Core_Complete_Final.exe" "..\output\development\tests\windows\"
...
copy /Y "Test_6Core_Complete_Final.exe" "..\windows_exe\"
...
copy /Y "Test_6Core_Complete_Final.exe" "C:\git\moe-charm\nyamesh_editor\test_exe\"

if %errorlevel% neq 0 (
    echo WARNING: Failed to copy executable to convenience directory
) else (
    echo SUCCESS: Executable copied to C:\git\moe-charm\nyamesh_editor\test_exe\Test_6Core_Complete_Final.exe
)

cd ..

echo.
echo === BUILD SUCCESS (SAFE VERSION) - 6 CORES COMPLETE FINAL INTEGRATION ===
echo Build directory: build-test-6core-complete-final-windows\Test_6Core_Complete_Final.exe
echo Output location: output\development\tests\windows\Test_6Core_Complete_Final.exe  
echo Legacy location: windows_exe\Test_6Core_Complete_Final.exe
echo Convenience location: C:\git\moe-charm\nyamesh_editor\test_exe\Test_6Core_Complete_Final.exe
echo.
echo SAFETY CONFIRMED: Root CMakeLists.txt was NOT modified
echo.
echo 6-Core Complete Final Integration Success:
...
echo.
echo ACHIEVEMENT: True 6-Core Intent-Driven Architecture Completed!
echo   - All cores use send/on communication patterns only
echo   - No direct API calls between cores
echo   - Complete header definition compliance
echo   - NyaMesh P2P message bus integration
echo   - Qt6 GUI with full functionality
echo.
echo How to run (multiple locations):
echo   cd output\development\tests\windows ^&^& Test_6Core_Complete_Final.exe
echo   cd windows_exe ^&^& Test_6Core_Complete_Final.exe  
