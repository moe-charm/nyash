@echo off
echo === NyaMesh Editor 6Core Integration Test Windows Build (SAFE VERSION) ===
...
if not exist build-test-6core-windows-safe mkdir build-test-6core-windows-safe
cd build-test-6core-windows-safe

echo Starting CMake configuration (SAFE MODE - No root overwrite)...

:: SAFE: Copy SAFE CMakeLists to BUILD directory (NOT root!)
copy /Y "..\CMakeLists_Test_6Core_Safe.txt" ".\CMakeLists.txt"
if %errorlevel% neq 0 (
...
:: Build execution
cmake --build . --config Release --target Test_6Core_Complete_Safe
...
copy /Y "Test_6Core_Complete_Safe.exe" "..\output\development\tests\windows\"
...
copy /Y "Test_6Core_Complete_Safe.exe" "..\windows_exe\"

cd ..

echo.
echo === BUILD SUCCESS (SAFE VERSION) - 6 CORES INTEGRATED ===
echo Build directory: build-test-6core-windows-safe\Test_6Core_Complete_Safe.exe
echo Output location: output\development\tests\windows\Test_6Core_Complete_Safe.exe  
echo Legacy location: windows_exe\Test_6Core_Complete_Safe.exe
echo.
echo SAFETY CONFIRMED: Root CMakeLists.txt was NOT modified
echo.
echo 6-Core Integration Complete:
...
echo   6. QuickAccessCore_v23_Integrated - Recent files and bookmarks management
echo.
echo How to run:
echo cd output\development\tests\windows
echo Test_6Core_Complete_Safe.exe
echo.
goto end
