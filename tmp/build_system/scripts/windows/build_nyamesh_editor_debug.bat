@echo off
REM NyaMesh Editor Debug Version Windows Build Script
echo === NyaMesh Editor v23 DEBUG BUILD ===
echo This will help identify the empty window issue
echo =============================================
...
echo Copying DEBUG CMake configuration...
copy /Y "%PROJECT_ROOT%build_system\cmake\targets\CMakeLists_NyaMeshEditor_Debug.txt" "CMakeLists.txt"
if %errorlevel% neq 0 exit /b 1

REM CMake configuration
echo Running CMake configuration for DEBUG build...
...
REM Build execution
echo Building NyaMesh Editor DEBUG version...
cmake --build . --config Debug --target NyaMeshEditorDebug
...
set TARGET_EXE=NyaMeshEditor_v23_Debug.exe
if not exist "%TARGET_EXE%" (
    echo ERROR: DEBUG target executable not found
    exit /b 1
)
...
echo Deploying NyaMesh Editor DEBUG version...
copy /Y "%TARGET_EXE%" "%OUTPUT_DIR%\"
if %errorlevel% neq 0 exit /b 1

REM Copy debug symbol files if they exist
if exist "NyaMeshEditor_v23_Debug.pdb" (
    echo Copying debug symbols...
    copy /Y "NyaMeshEditor_v23_Debug.pdb" "%OUTPUT_DIR%\"
)

REM Copy Qt6 dependencies (if not already there)
if not exist "%OUTPUT_DIR%\Qt6Core.dll" (
    echo Copying Qt6 DEBUG dependencies...
...
REM Create default configuration for debug
echo {"theme": "debug", "fontSize": 12, "language": "auto"} > "%OUTPUT_DIR%\editor_quickaccess_debug.json"
...
echo 1. Run the debug executable
echo 2. Check console output for detailed logs
echo 3. Look for [DEBUG] messages
echo 4. Check if test button appears
echo 5. Report findings for analysis
echo ===============================
