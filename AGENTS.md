...
#### 2.1 根治優先ポリシー（対処療法の禁止）
- 目標: 問題は「構造」と「契約（インターフェース）」で根治する。暫定コードの付け足しで誤差吸収しない。
- 原則:
  - Fail‑Fastを徹底（静かなフォールバック禁止）。エラーを隠さない・握りつぶさない。
  - パスの一時分岐や迂回で塞がない（上位で成立しても、下位層で腐食を招く）。
  - 性能は構造設計下で担保（大きな計算量増加を伴う迂回や全件再構築は禁止）。
- 例（禁止事項）:
  - Map.values 未整備→ keys 全列挙 + get ループで Array を自作（❌ 遅い・仕様逸脱）
  - Extern 失敗→ Method へ巻き戻し（❌ Fail‑Fast違反・原因隠蔽）
  - plugin 欠落を VM 側でごまかす（❌ 責務逆流）
- 例（正しい対応）:
  - 欠落した実体にパッチ（plugin/host_slot/extern_adapter の正道で修正）
  - Builder 正規化 + EmitGuard 一元化で素材化の順序と形状を保証
  - Verifier/Smokes で形状/挙動を固定（再発時に即時検知）
...
#### 3.1 一時的な緩和（例外の運用ルール）
- 目的: 開発を止めないための「短命・可逆」な緩和のみ許可。
- 条件（すべて満たすこと）:
  - 既定OFF（ENV/フラグで明示 opt‑in）。
  - TTL（撤去期限）を CURRENT_TASK.md に明記。撤去手順も併記。
  - Docs（本ファイルや Roadmap）に理由と影響範囲を記録。
  - Fail‑Fastを緩めない（エラーを隠蔽しない・ログで観測可能）。
- 許可例: 詳細ログの追加、dev 限定の観測トグル、perf 計測用ガード。
- 不許可例: 失敗時の自動フォールバック、仕様変更の恒常化、恒久的な遅い迂回路。
...
原則（強化）
- 静かなフォールバック禁止: 失敗は必ず Fail‑Fast。例: Extern が未登録なら即エラー（Method へ巻き戻さない）。
