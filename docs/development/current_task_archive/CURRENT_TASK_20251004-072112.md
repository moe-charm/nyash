...
## 構造設計
- [x] 新規フォルダ/ファイルが必要か → selfhost/vm/boxes/{instruction_scanner.hako, op_handlers.hako}
- [x] 各層のREADME.md更新が必要か → selfhost/vm/boxes/README に責務境界
...
- Mini‑VM 修正（selfhost/vm/boxes/mir_vm_min.hako）
...
- 次のステップ:
  ...
  4) quick 全緑確認
− 回避: インスタンスBuilder（selfhost/shared/json/mir_builder2.hako）を暫定使用
...
- Emit devトレース（最小、1行・ENV想定）を追加（現在は無条件出力。最後のJSON行でテストは影響なし）
  - selfhost/compiler/pipeline_v2/emit_compare_box.hako
  - selfhost/compiler/pipeline_v2/emit_return_box.hako
  - selfhost/compiler/pipeline_v2/emit_binop_box.hako
...
- 箱化（Extract）
  - 追加: `selfhost/compiler/pipeline_v2/compare_extract_box.hako`（Return/If Compare 抽出）
  - 追加: `selfhost/compiler/pipeline_v2/{call_extract_box,method_extract_box,new_extract_box}.hako`（整数引数のみ）
...
- Docs
  - 追加: `selfhost/compiler/pipeline_v2/README.md`（箱の契約/入出力/trace/フォールバック/wasm取込み方針）。
...
- `selfhost/compiler/pipeline_v2/backend_box.hako`（旧 .nyash）
- `selfhost/compiler/pipeline_v2/mir_builder_box.hako`（旧 .nyash）
...
- **ファイル**: `selfhost/vm/boxes/mir_vm_min.nyash` (Line 389-492)
...
3) Documentation: note added smokes in selfhost/vm/README.md and mention Div/Mod-by-zero + no-ret fallback policies
...
4) Ny 実行器 M2 スケルトン（最小） — baseline exists
   - Files: selfhost/vm/boxes/mir_vm_min.nyash; quick smoke present.
...
- Added Ny-based minimal MIR(JSON v0) executor skeleton (const→ret only), dev-only app — no default behavior change.
  - File: selfhost/vm/boxes/mir_vm_min.nyash
  - Entry: selfhost/vm/mir_min_entry.nyash (optional thin wrapper)
...
- M2 executor (Ny):
  ...
  - Externalize MirVmM2 to selfhost/vm/boxes/mir_vm_m2.nyash and switch smoke to using-based variant; keep inline smoke as safety. [later]
...
- Drafted NYABI Kernel spec (v0) and added Ny skeleton box (not wired).
  - docs/abi/vm-kernel.md; selfhost/vm/boxes/vm_kernel_box.nyash
...
- Fix: compare/ret segmentation made robust without heavy JSON parse.
  ...
  - Files: selfhost/vm/boxes/mir_vm_min.nyash
...
- Mini‑VM（selfhost/vm/boxes/mir_vm_min.hako）を強化
...
- Emit 側（selfhost/compiler/pipeline_v2/emit_compare_box.hako）
