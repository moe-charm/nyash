...
Contribution (Nyash)
- Express try/catch/finally with Result‑mode lowering (no MIR Throw/Catch): structured blocks and jumps.
...
- Block‑Postfix Catch syntax: `{ body } catch (e) { … } [finally { … }]` with single‑catch policy.
...
- Finally: always runs; merge at exit with PHI‑off rules.
...
- Control‑flow complexity: nested if/loop + finally; ensure merges are stable.
