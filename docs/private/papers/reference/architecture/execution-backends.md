# Nyash実行バックエンド完全ガイド（推奨4経路 + 任意 JIT）

Nyashプログラミング言語は、**Everything is Box**哲学を維持しながら、推奨4経路（Interpreter / VM / Cranelift AOT / LLVM AOT）を中心に運用します。任意で Cranelift JIT ランタイムを有効化できます。
