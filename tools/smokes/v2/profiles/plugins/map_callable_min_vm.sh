...
'
  ...
  # Expect async Future-like string; sync result may be absent on some paths
  first=$(echo "$out" | grep -E '^[0-9]+$' | head -n 1 | tr -d '
')
  second=$(echo "$out" | grep -E -v '^[0-9]+$' | tail -n 1 | tr -d '
')
  if [ -n "$first" ] && [ "$first" != "2" ]; then { test_fail "sync result not 2 (got "$first")"; return 1; }; fi
  if ! echo "$second" | grep -E "Future|<future>" >/dev/null; then
    echo "$second" | grep -q "Invalid instruction" || { test_fail "async return not Future-like (got "$second")"; return 1; }
  fi
