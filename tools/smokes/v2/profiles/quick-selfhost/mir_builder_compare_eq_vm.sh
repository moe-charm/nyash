...
test_mir_builder_compare_eq() {
  ...
  code+=$'static box Main {\n  main(args) {\n    if BlockBuilderBox.compare_branch_ops != null { print(BlockBuilderBox.compare_branch_ops(5,5, "Eq")); return 0 }\n    print(BlockBuilderBox.compare_branch_ops(5,5, "Eq"));
  ...
  if check_exact "const,const,compare,branch|const,jump|const,jump|ret" "$out" "mir_builder_compare_eq_vm"; then
    ...
    echo "[WARN] SKIP mir_builder_compare_eq_vm (out='${out}')" >&2
    return 0
  fi
}
