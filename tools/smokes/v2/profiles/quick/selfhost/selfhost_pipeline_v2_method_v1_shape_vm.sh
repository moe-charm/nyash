#!/bin/bash
# selfhost_pipeline_v2_method_v1_shape_vm.sh — Pipeline V2 v1(Method) shape (VM-only)
...
TMP_DIR="/tmp/selfhost_pipeline_v2_method_v1_shape_vm_$$"
mkdir -p "$TMP_DIR"
...
static box Main {
  main() {
    ...
    if j.indexOf("\"type\":\"Method\"") < 0 { print("ng2") return 1 }
    if j.indexOf("\"receiver\":1") < 0 { print("ng3") return 1 }
    print("ok")
    return 0
  }
}
...
compare_outputs "ok" "$out" "selfhost_pipeline_v2_method_v1_shape_vm" || { cd /; rm -rf "$TMP_DIR"; exit 1; }
