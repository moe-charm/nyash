#!/bin/bash
# selfhost_pipeline_v2_ret_vm.sh — Pipeline V2: Stage‑1 Return(Int) → MIR(JSON) → Mini‑VM
...
TMP_DIR="/tmp/selfhost_pipeline_v2_ret_vm_$$"
mkdir -p "$TMP_DIR"
...
static box Main {
  main() {
    // Stage‑1 JSON: Return(Int 123)
    local ast = "{\"version\":0,\"kind\":\"Program\",\"body\":[{\"type\":\"Return\",\"expr\":{\"type\":\"Int\",\"value\":123}}]}"
...
expected="123"
compare_outputs "$expected" "$out" "selfhost_pipeline_v2_ret_vm" || { cd /; rm -rf "$TMP_DIR"; exit 1; }
