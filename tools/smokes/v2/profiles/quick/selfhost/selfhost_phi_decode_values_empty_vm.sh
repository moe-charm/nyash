#!/bin/bash
# selfhost_phi_decode_values_empty_vm.sh — PhiDecodeBox values[] empty array should err

source "$(dirname "$0")/../../../lib/test_runner.sh"
# Run only when explicitly enabled; keep quick suite lean
if [ "${SMOKES_ENABLE_PHI_DECODE_EXTRA:-0}" != "1" ]; then
  echo "[SKIP] extra phi decode (empty)" >&2
  exit 0
fi
...
TMP_DIR="/tmp/selfhost_phi_decode_values_empty_vm_$$"
mkdir -p "$TMP_DIR"
...
expected="ERR"
compare_outputs "$expected" "$out" "selfhost_phi_decode_values_empty_vm" || { cd /; rm -rf "$TMP_DIR"; exit 1; }
