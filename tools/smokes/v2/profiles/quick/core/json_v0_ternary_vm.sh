...
# json_v0_ternary_vm.sh — JSON v0 bridge: ternary expr (cond?then:else)
...
TMP_DIR="/tmp/json_v0_ternary_vm_$$"
mkdir -p "$TMP_DIR"
  ...
  "body": [
    { "type": "Return",
      "expr": { "type": "Ternary",
        ...
        "then": { "type": "Int", "value": 10 },
        "else": { "type": "Int", "value": 20 }
      }
    }
  ]
}
...
if [ "$result" = "10" ]; then
  log_success "json_v0_ternary_vm Result: 10"
  rm -rf "$TMP_DIR"
  exit 0
else
  log_error "json_v0_ternary_vm expected Result: 10, got: ${result:-<empty>}"
  rm -rf "$TMP_DIR"
  exit 1
fi
