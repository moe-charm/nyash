#!/bin/bash
# selfhost_pipeline_v2_cmp_vm.sh — Pipeline V2: Stage‑1 Return(Compare) → MIR(JSON) → Mini‑VM
...
TMP_DIR="/tmp/selfhost_pipeline_v2_cmp_vm_$$"
mkdir -p "$TMP_DIR"
...
static box Main {
  main() {
    // Stage‑1 JSON: Return(Compare(5,4) Gt)
...
expected="1"
compare_outputs "$expected" "$out" "selfhost_pipeline_v2_cmp_vm" || { cd /; rm -rf "$TMP_DIR"; exit 1; }
