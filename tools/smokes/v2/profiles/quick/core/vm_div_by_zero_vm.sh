#!/bin/bash
# vm_div_by_zero_vm.sh — Division by zero should raise a VM error (no output)
...
if [ "${SMOKES_ENABLE_ROOTFIX:-0}" != "1" ]; then
  test_skip "vm_div_by_zero_vm (root-fix WIP)" "Enable with SMOKES_ENABLE_ROOTFIX=1" || exit 0
  exit 0
fi

TMP_DIR="/tmp/vm_div_by_zero_vm_$$"
mkdir -p "$TMP_DIR"
