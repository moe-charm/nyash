...
# json_v0_extern_console_log_vm.sh — JSON v0 bridge: Extern console.log
...
TMP_DIR="/tmp/json_v0_extern_console_log_vm_$$"
mkdir -p "$TMP_DIR"
...
cat > "$JSON_FILE" << 'EOF'
{
  ...
  "kind": "Program",
  "body": [
    { "type": "Extern", "iface": "env.console", "method": "log", "args": [ { "type": "Str", "value": "hi" } ] },
...
if [ "$result" = "0" ]; then
  log_success "json_v0_extern_console_log_vm Result: 0"
  rm -rf "$TMP_DIR"
  exit 0
else
  log_error "json_v0_extern_console_log_vm expected Result: 0, got: ${result:-<empty>}"
  rm -rf "$TMP_DIR"
  exit 1
fi
