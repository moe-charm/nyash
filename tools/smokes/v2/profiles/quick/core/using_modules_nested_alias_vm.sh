...
if [ "${SMOKES_ENABLE_NESTED_ALIAS:-0}" != "1" ]; then
  test_skip "using_modules_nested_alias_vm (needs update after JSON reorg)" "Enable with SMOKES_ENABLE_NESTED_ALIAS=1" || exit 0
  exit 0
fi
