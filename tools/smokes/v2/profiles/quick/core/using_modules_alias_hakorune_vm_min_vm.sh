...
if run_nyash_vm "$SRC" >/dev/null; then
  log_success "using_modules_alias_hakorune_vm_min_vm resolved hakorune.vm.mir_min"
  rm -rf "$TMP_DIR"
  exit 0
else
  log_error "using_modules_alias_hakorune_vm_min_vm failed (non-zero rc)"
  rm -rf "$TMP_DIR"
  exit 1
fi
