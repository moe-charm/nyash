#!/usr/bin/env bash
# Quick smoke: extern string len_h via length() on literal (expects Result: 2)
...
APP="apps/tests/extern_string_len_h.nyash"
...
[[ "$line" == *"Result: 2"* ]] && test_pass "$name" || { echo "$out" >&2; test_fail "$name" "expected 'Result: 2'" && exit 1; }
