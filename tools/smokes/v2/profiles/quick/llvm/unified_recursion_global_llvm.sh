#!/usr/bin/env bash
# unified_recursion_global_llvm.sh — Unified Call recursion (Global function)
...
if [[ "${SMOKES_ENABLE_UNIFIED_RECURSION:-}" != "1" ]]; then
  test_skip "Unified recursion (Global) gated; set SMOKES_ENABLE_UNIFIED_RECURSION=1"
  exit 0
fi
...
TMP_DIR="/tmp/unified_recursion_global_llvm_$$"
mkdir -p "$TMP_DIR"
...
OUT="/tmp/unified_recursion_global_bin_$$"
...
compare_outputs "$exp" "$act" "unified_recursion_global_llvm" || { cd /; rm -rf "$TMP_DIR"; exit 1; }
