#!/usr/bin/env bash
# unified_recursion_modulefn_llvm.sh — Unified Call recursion (ModuleFunction static box)
...
if [[ "${SMOKES_ENABLE_UNIFIED_RECURSION:-}" != "1" ]]; then
  test_skip "Unified recursion (ModuleFunction) gated; set SMOKES_ENABLE_UNIFIED_RECURSION=1"
  exit 0
fi
...
TMP_DIR="/tmp/unified_recursion_modulefn_llvm_$$"
mkdir -p "$TMP_DIR"
...
OUT="/tmp/unified_recursion_modulefn_bin_$$"
...
compare_outputs "$exp" "$act" "unified_recursion_modulefn_llvm" || { cd /; rm -rf "$TMP_DIR"; exit 1; }
