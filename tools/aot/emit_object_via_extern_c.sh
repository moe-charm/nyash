#!/usr/bin/env bash
# Emit an object file from MIR JSON via extern_c + libllvm_backend
# Usage: tools/aot/emit_object_via_extern_c.sh <mir.json> <out.o>

set -euo pipefail

if [ $# -ne 2 ]; then
  echo "Usage: $0 <mir.json> <out.o>" >&2
  exit 2
fi
...
if ! echo "$HAKO_FFI_ALLOW_LIST" | grep -q '\<llvm_compile_mir_to_object\>'; then
  export HAKO_FFI_ALLOW_LIST=$(echo "${HAKO_FFI_ALLOW_LIST:+$HAKO_FFI_ALLOW_LIST,}llvm_compile_mir_to_object")
fi
...
TMP_SRC="/tmp/hako_emit_obj_$$.hako"
cat > "$TMP_SRC" << 'HK'
static box Main {
  main() {
...
# Inline paths
sed -i -e "s@__IN__@${MIR_JSON}@g" -e "s@__OUT__@${OUT_O}@g" "$TMP_SRC"
