...
export NYASH_MACRO_BOX_NY_PATHS="apps/macros/examples/echo_macro.nyash"
...
if [ "$(norm "$out")" != "$(norm "$(cat "$golden")")" ]; then
  echo "Golden mismatch (user macro identity)" >&2
  diff -u <(echo "$out") "$golden" || true
  exit 2
fi

echo "[OK] golden user macro identity matched"
