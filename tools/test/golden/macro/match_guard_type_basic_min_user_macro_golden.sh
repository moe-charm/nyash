...
src="apps/tests/macro/match/guard_type_basic_min.nyash"
golden="$root/tools/test/golden/macro/match_guard_type_basic_min.expanded.json"
...
if [ "$out_norm" != "$gold_norm" ]; then
  echo "Golden mismatch (match guard type basic min)" >&2
  diff -u <(echo "$out_norm") <(echo "$gold_norm") || true
  exit 2
fi

echo "[OK] golden match guard type basic min matched"
