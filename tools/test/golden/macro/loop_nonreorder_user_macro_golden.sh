...
src="apps/tests/macro/loopform/nonreorder.nyash"
golden="$root/tools/test/golden/macro/loop_nonreorder.expanded.json"
...
if [ "$out_norm" != "$gold_norm" ]; then
  echo "[FAIL] loop_nonreorder expanded JSON mismatch" >&2
  diff -u <(echo "$out_norm") <(echo "$gold_norm") || true
  exit 2
fi

echo "[OK] golden loop_nonreorder expansion matched"
