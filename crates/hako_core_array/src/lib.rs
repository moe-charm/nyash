...
pub fn length(len: usize) -> i64 { len as i64 }
...
pub fn slice_bounds(len: usize, start: i64, end: i64) -> (usize, usize) {
    ...
    let i1 = if end < 0 { len } else { end.max(0).min(l) as usize };
    if i0 > i1 {
        i0 = i1;
    }
    (i0, i1)
...
pub enum SetIndex { Replace(usize), Append, Oob }

/// Classify index for set() according to core array semantics.
pub fn classify_set_index(len: usize, idx: i64) -> SetIndex {
    if idx < 0 { return SetIndex::Oob; }
    let u = idx as usize;
    if u < len { SetIndex::Replace(u) }
    else if u == len { SetIndex::Append }
    else { SetIndex::Oob }
}
