...
pub extern "C" fn nyash_plugin_invoke_by_name_tagged_v_i64(
    ...
    if invoke.is_none() {
        if trace { eprintln!("nyrt: vinvoke.by_name: resolve recv failed (recv_h={})", recv_handle); }
        return 0;
    }
    let box_type = box_type.unwrap_or_default();
...
pub extern "C" fn nyash_future_spawn_method_h(
    ...
    if trace {
        eprintln!(
            "nyrt: vinvoke.spawn: type_id={} method_id={} recv_h={} argc={} vals_ptr={:p} tags_ptr={:p}",
            type_id, method_id, recv_h, argc, vals, tags
        );
    }
    if invoke.is_none() { return 0; }
    ...
    if trace {
        let sample = std::cmp::min(nargs, 8);
        eprintln!(
            "nyrt: vinvoke.spawn: real_type_id={} nargs={} tags[..{}]={:?}",
            real_type_id, nargs, sample, &tags_slice[..sample]
        );
    }
    for i in 0..nargs {
    ...
    nyash_rust::runtime::global_hooks::spawn_task("nyash.future.spawn_method_h", Box::new(move || {
        ...
        if trace { eprintln!("nyrt: vinvoke.spawn: rc={} out_len={}", rc, out_len); }
        if rc != 0 {
        ...
        if let Some((tag, sz, payload)) = nyash_rust::runtime::plugin_ffi_common::decode::tlv_first(slice) {
            if trace { eprintln!("nyrt: vinvoke.spawn: ret_tag={}", tag); }
            match tag {
                3 => { // I64
                    if payload.len() == 8 {
                        let mut b = [0u8;8]; b.copy_from_slice(payload); let n = i64::from_le_bytes(b);
...
// (string concat helpers moved to module string_concat)
