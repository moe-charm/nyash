...
impl Arena {
    fn new() -> Self { Self { next: AtomicI64::new(1), objs: Mutex::new(HashMap::new()), cstr: Mutex::new(HashMap::new()) } }
    ...
    fn get(&self, h: i64) -> Option<ObjRef<'_>> {
        ...
        Some(ObjRef { _guard: guard, obj: unsafe { (*ptr).get(&h) } })
    }
    ...
    fn str_ptr(&self, h: i64) -> *const i8 {
        if let Some(ObjRef { obj: Some(Obj::Str(s)), .. }) = self.get(h) {
...
pub extern "C" fn nyash_box_from_i8_string(ptr_in: *const i8) -> i64 {
    if !min_sem_enabled() { return 0; }
    if ptr_in.is_null() { return 0; }
    let s = unsafe { CStr::from_ptr(ptr_in) }.to_string_lossy().to_string();
...
pub extern "C" fn nyash_string_to_i8p_h(h: i64) -> *const i8 {
    if !min_sem_enabled() { return ptr::null(); }
    arena().str_ptr(h)
}
...
pub extern "C" fn nyash_string_from_u64x2(lo: u64, _hi: u64, len: u64) -> i64 {
    if !min_sem_enabled() { return 0; }
    if len == 0 { return arena().alloc(Obj::Str(String::new())); }
...
pub extern "C" fn nyash_string_len_h(h: i64) -> i64 {
    if !min_sem_enabled() { return 0; }
    match arena().get(h).and_then(|r| r.obj) { Some(Obj::Str(s)) => s.len() as i64, _ => 0 }
}
...
pub extern "C" fn nyash_string_charcodeat_h(h: i64, idx: i64) -> i64 {
    if !min_sem_enabled() { return -1; }
    if idx < 0 { return -1; }
    match arena().get(h).and_then(|r| r.obj) {
        Some(Obj::Str(s)) => {
            let i = idx as usize;
            if i < s.len() { s.as_bytes()[i] as i64 } else { -1 }
        }
        _ => -1,
    }
}
...
pub extern "C" fn nyash_string_concat_hh(a: i64, b: i64) -> i64 {
    if !min_sem_enabled() { return 0; }
    let sa = match arena().get(a).and_then(|r| r.obj) { Some(Obj::Str(s)) => s.clone(), _ => String::new() };
    let sb = match arena().get(b).and_then(|r| r.obj) { Some(Obj::Str(s)) => s.clone(), _ => String::new() };
...
pub extern "C" fn nyash_string_eq_hh(a: i64, b: i64) -> i64 {
    if !min_sem_enabled() { return 0; }
    let sa = match arena().get(a).and_then(|r| r.obj) { Some(Obj::Str(s)) => s, _ => return 0 };
    let sb = match arena().get(b).and_then(|r| r.obj) { Some(Obj::Str(s)) => s, _ => return 0 };
    if sa == sb { 1 } else { 0 }
}
...
pub extern "C" fn nyash_string_lastindexof_hh(h: i64, n: i64) -> i64 {
    if !min_sem_enabled() { return -1; }
    match (arena().get(h).and_then(|r| r.obj), arena().get(n).and_then(|r| r.obj)) {
        (Some(Obj::Str(hs)), Some(Obj::Str(ns))) => hs.rfind(ns.as_str()).map(|i| i as i64).unwrap_or(-1),
        _ => -1,
    }
}
...
pub extern "C" fn nyash_array_new() -> i64 { nyash_array_birth_h() }
#[export_name = "nyash.map.new"]
pub extern "C" fn nyash_map_new() -> i64 { nyash_map_birth_h() }

// Optional env API used by some builder paths (new box via type string)
#[export_name = "nyash.env.box.new_i64x"]
pub extern "C" fn nyash_env_box_new_i64x(typ: *const i8, _argc: i64, _a1: i64, _a2: i64, _a3: i64, _a4: i64) -> i64 {
    if !min_sem_enabled() { return 0; }
    if typ.is_null() { return 0; }
...
pub extern "C" fn nyash_array_push_h(arr_h: i64, val_h: i64) -> i64 {
    ...
    if let Some(mut guard) = arena().get_mut(arr_h) {
        if let Some(Obj::Arr(v)) = guard.get_mut(&arr_h) {
            v.push(val_h);
            if std::env::var("HAKO_DEBUG").ok().as_deref() == Some("1") {
                eprintln!("[hako_kernel] array.push h={} val={} -> len={}", arr_h, val_h, v.len());
            }
            return v.len() as i64;
        }
    }
...
pub extern "C" fn nyash_map_set_hh(map_h: i64, key_h: i64, val_h: i64) -> i64 {
    if !min_sem_enabled() { return 0; }
    let key_s = match arena().get(key_h).and_then(|r| r.obj) { Some(Obj::Str(s)) => s.clone(), _ => format!("{}", key_h) };
...
pub extern "C" fn nyash_map_has_hh(map_h: i64, key_h: i64) -> i64 {
    if !min_sem_enabled() { return 0; }
    let key_s = match arena().get(key_h).and_then(|r| r.obj) { Some(Obj::Str(s)) => s.clone(), _ => format!("{}", key_h) };
    if let Some(ObjRef { obj: Some(Obj::Map(m)), .. }) = arena().get(map_h) {
...
pub extern "C" fn nyash_map_get_hh(map_h: i64, key_h: i64) -> i64 {
    ...
    let key_s = match arena().get(key_h).and_then(|r| r.obj) { Some(Obj::Str(s)) => s.clone(), _ => format!("{}", key_h) };
    if let Some(ObjRef { obj: Some(Obj::Map(m)), .. }) = arena().get(map_h) {
        if let Some(vh) = m.get(&key_s) { return *vh; }
    }
    0
}
