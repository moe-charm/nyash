...
"#;
    ...
    #[derive(Default)]
    struct Item { name: String, argc: usize, io: bool }
    ...
    buf.push_str("pub fn generated_externs() -> &'static [(&'static str, usize, bool)] {\n    &[\n");
    for it in &items {
        if it.name.is_empty() { continue; }
        buf.push_str(&format!("        (\"{}\", {}, {}),\n", it.name, it.argc, if it.io { "true" } else { "false" }));
    }
    buf.push_str("    ]\n}\n");

    let mut f = fs::File::create(&out_file).expect("create generated externs file");
    f.write_all(buf.as_bytes()).expect("write generated externs");
}
