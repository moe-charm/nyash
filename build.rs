...
"#;
    ...
    generated.push_str("// Auto-generated by build.rs — static plugins registry
");
    generated.push_str("// Edit static_plugins.toml to change the set.

");
    generated.push_str("pub fn register_static_plugins() {
");
    if static_cfg.exists() {
        println!("cargo:rerun-if-changed={}", static_cfg.display());
        let cfg_str = std::fs::read_to_string(&static_cfg)
            .expect("Failed to read static_plugins.toml");
        #[derive(serde::Deserialize)]
        struct PluginCfg { path: String, #[allow(dead_code)] feature: Option<String> }
        #[derive(serde::Deserialize)]
        struct Root { plugins: Vec<PluginCfg> }
